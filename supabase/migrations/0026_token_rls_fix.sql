-- Fix token debits: RLS blocked ledger inserts inside SECURITY DEFINER functions.
-- Drop old signatures (return type may differ across prior migrations) and recreate with row_security = off.
...
drop function if exists public.profiles_sanitize_client_updates();
...
-- Service role may write ledger rows via API fallback.
drop policy if exists "token_transactions_service_write" on public.token_transactions;
create policy "token_transactions_service_write" on public.token_transactions
  for all
  to service_role
  using (true)
  with check (true);
